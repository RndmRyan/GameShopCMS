<script setup lang="ts">
  import { ref } from 'vue'
  import { usePlayersAPI } from '@/composables/apis'
...
const headers = ref([
  ...
  { title: 'Wallet', key: 'wallet', align: 'end' },
  { title: 'Last Updated', key: 'last_updated', align: 'end' },
...
async function loadItems({ page, itemsPerPage, sortBy }: { page: number, itemsPerPage: number, sortBy: any }) {
  ...
  const { players } = await usePlayersAPI()
  let items = players
...
<template>
    <v-data-table-server
      ...
      <template #item.wallet="{ item }">
        <pre>{{ JSON.stringify(item.wallet, null, 2) }}</pre>
      </template>
